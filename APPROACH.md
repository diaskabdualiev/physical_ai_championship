...
2. **APPROACHING** — куб обнаружен:
   ...
   - **Остановка**: когда горизонтальная дистанция до куба <= 0.5м, робот останавливается
...
## Оценка расстояния
Камера `head_cam` наклонена вниз (~56° от горизонтали, `xyaxes="0 -1 0 1.5 0 1"`). Depth buffer даёт расстояние вдоль оптической оси камеры, а не горизонтальную дистанцию до куба. Для корректного порога остановки выполняется пересчёт:

1. По позиции пикселя куба и FOV камеры (75°) вычисляется направление луча в системе координат камеры
2. Луч трансформируется в систему координат тела робота через матрицу поворота камеры
3. Горизонтальная проекция (X, Y) даёт реальное расстояние на уровне земли
4. Робот останавливается когда горизонтальная дистанция ≤ 0.5м

Это гарантирует корректную остановку независимо от высоты камеры и её наклона.
