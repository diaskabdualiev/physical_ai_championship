...
### Режим циклического тестирования (auto_respawn)

Для проверки устойчивости решения к разным позициям куба и ориентациям робота можно включить автоматический респавн. После того как робот находит куб и останавливается, через заданное время оба сбрасываются в новые случайные позиции и цикл повторяется бесконечно.

В `config.yaml`:
```yaml
auto_respawn: 1          # включить циклический респавн
auto_respawn_delay: 2.0  # секунды ожидания в STOPPED перед респавном
```

**Цикл:** поиск → подход → остановка → 2с пауза → респавн робота (центр, новый yaw) + куба (случайная позиция) → поиск → ...
